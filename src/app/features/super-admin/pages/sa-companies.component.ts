...
import { numeroALetras } from '../../../shared/utils/number-to-words.util';
interface Company {
  id: string;
  name: string;
  nit: string;
...
export class SaCompaniesComponent implements OnInit {
  ...
  min(a: number, b: number) { return Math.min(a, b); }
    ...
    totalEnLetras = computed(() => {
      ...
      const texto = numeroALetras(total, 'femenino');

      if (total === 1) {
        return `${texto} empresa registrada en la plataforma`;
      }

      return `${texto} empresas registradas en la plataforma`;
    });
}